...
# ╔═╡ 0b76fce9-c0dd-4e34-9c52-25724649065d
using Plots
...
begin
	function newton(f, f_prime, x0, n)               
		raiz_aproximada= (x0 - (f(x0)/f_prime(x0)))      #Ecuación de Newton-Raphson
			for i in 1:n                                 #Ciclo por un número de iteraciones
				raiz_aproximada= raiz_aproximada - (f(raiz_aproximada)/f_prime(raiz_aproximada))
		    end
			println("Aproximación de la raíz:",raiz_aproximada)
	end
end	

# ╔═╡ 34688688-68ba-43b1-b2f2-7e7a79178e5d
# Definiciones
begin
	h(x)= x^2 - 2
	h_prime(x)= 2x        # Definición de f_prime(x)
	x0 = 1.5              # Aproximación inicial y número de iteraciones
end          


# ╔═╡ 3e7b4b3f-46cd-44a0-ba4f-a4a7941d7eb5
newton(h, h_prime, x0, 4)
...
# Tu código (comentado) va aquí :D
begin
	function newtonRecursiva(f, f_prime, x0, n)
		raiz_aproximada=(x0 - (f(x0)/f_prime(x0)))#Ecuación de Newton-Raphson
	    n-=1                                      #Como es recursiva queremos que se reste 1
	    if n!=0                                   #El ciclo se repite hasta llegar a n=0
			newtonRecursiva(f, f_prime, raiz_aproximada, n) #Tomamos la raiz aproximada
		end
...
# ╔═╡ 8cc2ff9c-8e83-4b3c-a219-cf94b1798305
newtonRecursiva(h, h_prime, x0, 8)
...
# Tu código (comentado) va aquí :D
begin
	function newtonDistancia(f,f_prime,x0,epsilon )
	if epsilon<=0                                     #épsilon debe ser mayor a cero
		print("Error, el valor ingresado es menor o igual que cero")
	end
	raiz_anterior = x0
	raiz_actual = x0 - (f(x0) / f_prime(x0))
	distancia = abs(raiz_actual - raiz_anterior)
		while distancia > epsilon
        raiz_anterior = raiz_actual
        raiz_actual = raiz_anterior - (f(raiz_anterior) / f_prime(raiz_anterior))
        distancia = abs(raiz_actual - raiz_anterior)
		end
	end
end

# ╔═╡ 0fe19f72-d96d-4f7a-87eb-7c6019f6dfd5
newtonDistancia(h, h_prime, x0, -8)

# ╔═╡ 5c6c3094-fea7-4aea-9e60-d9d356bb3442
newtonDistancia(h, h_prime, x0, 4)
...
# ╔═╡ 44877569-d5c2-42f5-b715-bfef4605185a


# ╔═╡ 6161868d-da06-40d3-b7df-b27887aa2778
...
# ╔═╡ ef7b2d44-c9a6-4edf-97ba-95ecd453bc8a
newtonDN(j',10, 6)
...
# ╔═╡ e49b32df-866f-4bd7-a4a4-8effb3b0bf22


# ╔═╡ 8b2238dd-3760-43b3-9d5c-d795c67b7797


# ╔═╡ 2516edac-19cb-4e87-ab08-9be273313830


# ╔═╡ c200ae92-5a58-47a8-9659-5d8d4f6a68d9
...
x_values = [x01]
	y_values = [f(x01)]
		for i in 1:n
		x0 = newtonDN(f'1, x01, h1)
			push!(x_values, x01)
				push!(y_values, f(x01)
				end

# Graficar la función y los puntos
plot(f, 0, 2π, label="f(x)=cos(x)", legend=:bottomright)
	scatter!(x_values, y_values, markersize=2, label="Puntos")
		xlabel!("x")
		ylabel!("f(x)")
		title!("Gráfica de f(x) = cos(x) con puntos (xi, f(xi))")
