...
# ╔═╡ 03041026-b721-4057-8593-ab1117d13de4
using Plots
...
# ╔═╡ 6e6c1aad-2016-4195-b305-aa034d6bef58
rand()


# ╔═╡ 70161fce-cdfc-4368-b6c4-8b67fd926cc4
md"""Nos da número aleatorio en un rango de 0 a 1"""

# ╔═╡ 84f102e6-2da4-485c-9519-a9ce4155c9c4
rand(Int)

# ╔═╡ 814a936b-87d0-407a-a15a-149c5e77d0d6
md""" Genera un entero al azar del mismo tipo al que está ligado Int"""

# ╔═╡ fa6a5638-77c0-40c3-8030-7917a7bbd9f9
rand(Bool)

# ╔═╡ 3417132f-9d60-4d48-a2c1-8e8f90352ad7
md""" Valores boleanos, cada valor tiene probabilidad de 50%"""
...
# ╔═╡ 92d50b23-485d-4f87-b42b-29d365239ce7
rand(n)

# ╔═╡ 2c38c396-8326-4f28-bbae-fba37108e07f
md"""Se crea un arreglo de n números de tipo flotante"""

# ╔═╡ a7aca8d4-d7a1-42e5-a245-99065b722923
rand(Int,n-1)

# ╔═╡ 4d4638ba-b09f-43d9-b34b-03fc257ccd7f
rand(Bool,n)
...
# Tu código va aquí :)
begin
 function aleatorioUniforme(a, b)                   #definimos la función
       numero_aleatorio=( a + (b - a) * rand() )
	 print(numero_aleatorio)                        #Que imprima el número
 end
end


# ╔═╡ 11e08687-510b-4dc3-9ff2-f9c1e0a77813
aleatorioUniforme(15, 30)
...
function Histograma(a, b)
     ...
     N_aleatorios= [aleatorioUniforme(a, b) for _ in 1:5]   #Generar 5 números aleatorios

    histogram(N_aleatorios, legend=false,                   # Crear histograma
              xlabel="Número aleatorio", ylabel="Frecuencia",
              title="Distribución de aleatorioUniforme")
end
end

# ╔═╡ 19b8a780-d38d-4181-bb9d-31cb8351b409
Histograma(1, 5)
...
begin 
    ...
    if rand(Bool)            #generar un valor booleano aleatorio
        return -1             
	else                     #condicional if para asignar 1(false) y -1(true) al elemento en 
        return 1             #el arreglo resultado       
     end
	end
end	

# ╔═╡ 86abe65e-956d-47ca-b2f5-efd81bd164cf
unPaso()
...
# Tu código va aquí :)
begin
	function variosPasos(n::Int)
    valor = Array{Int}(undef, n)     #creamos un arreglo resultado de tamaño n para 
    	for i in 1:n                   #almacenar los valores generados.      
       	 if rand(Bool)               #Iteramos n veces utilizando un bucle for.
            valor[i] = 1             #Asignamos 1 o -1 al elemento correspondiente 
		 else                           #en el arreglo resultado.
            valor[i] = -1
        	end
   		 end
    	return valor              #retornamos el arreglo resultado con los valores generados
	end
end	

# ╔═╡ 540dfcc1-fc31-46ad-a11f-b3d9fb66442a

	entradas = variosPasos(20)
...
# Tu código (comentado) va aquí :)
begin
function caminataAleatoria(n::Int)         #Creamos un arreglo caminata de tamaño n+1 para
    caminata = Array{Int}(undef, n+1)      #guardar las posiciones de la caminata aleatoria.
    posicion_inicial = 0
    caminata[1] = posicion_inicial
    
    for i in 2:n+1                         #Bucle for para simular los pasos.
        if rand(Bool)                      #Cada paso generamos un valor booleano al azar
            posicion_inicial += 1
        else
            posicion_inicial -= 1
        end
        caminata[i] = posicion_inicial
    end
    
    return caminata                        #Contiene todas las posiciones de la caminata 
end
end	

# ╔═╡ a0bf2094-9ca4-44bb-acf7-4e169f2f0b63
N_pasos = 15


# ╔═╡ 21d2c7f7-45f5-4705-952f-d93c665f1d16

simulación_de_caminata = caminataAleatoria(N_pasos)
...
# Tu código (comentado) va aquí :)
begin
function graficaCaminata(simulación_de_caminata::Array{Int}) 
	#Toma como entrada un arreglo simulación_de_caminata
	                                                     
    num_pasos = length(simulación_de_caminata)      #como la longitud del arreglo                        
    posiciones = simulación_de_caminata             #Asignamos el arreglo a posiciones para 
                                        #utilizarlo como los valores del eje y en la gráfica
  
	scatter(1:num_pasos, posiciones, legend = false, 
		    xlabel = "Cantidad de pasos", ylabel = "Posición")
end
end

# ╔═╡ e3e82731-db40-4b5d-aa31-26a27966b1d9
graficaCaminata(simulación_de_caminata)

# ╔═╡ a362f31b-4e59-4f28-9bbe-2eff8977cbe4
n2 = 15  # Define el valor de n2 como un número entero positivo

# ╔═╡ 6c37b712-af97-40e8-9b5f-127d9edd8cf0
simulación_de_caminata2 = caminataAleatoria(n2)  # Genera una caminata aleatoria usando la función caminataAleatoria

# ╔═╡ b308c403-4150-40df-8275-65b1789a23d4
graficaCaminata(simulación_de_caminata2)  
# Genera la gráfica de la caminata aleatoria utilizando la función graficaCaminata para simulación_de_caminata2
...
# ╔═╡ c30c654a-af73-4ef8-95a6-83dba268f59d
# Tu código (comentado) va aquí :)
begin
function graficaCaminata!(simulación_de_caminata3::Array{Int}) 
	#Toma como entrada un arreglo simulación_de_caminata
	                                                     
    num_pasos = length(simulación_de_caminata3)      #como la longitud del arreglo                        
    posiciones = simulación_de_caminata3             #Asignamos el arreglo a posiciones para 
                                        #utilizarlo como los valores del eje y en la gráfica
  
	plot(1:num_pasos, posiciones, legend = false, linestyle=:dash, #Lineas rectas punteadas
		    xlabel = "Cantidad de pasos", ylabel = "Posición")
end
end

# ╔═╡ 3e8880cc-a4af-4a2b-8c10-4ce92bfefd2b
n3 = 7  # Número de pasos para cada caminata

# ╔═╡ 4ef168c6-c1ee-4dd4-8523-71ada9a6a3eb
begin
for _ in 1:5
	simulación_de_caminata3 = caminataAleatoria(n3)
    graficaCaminata!(simulación_de_caminata3)	
end
end
